...
All Dart models live in `lib/data/models/`. Each model that maps to Firestore has a `fromFirestore(DocumentSnapshot)` factory and a `toFirestore()` method. Models: `AppUser`, `BoardGame`, `StoreInfo`, `WatchlistItem`, `PriceHistoryEntry`, `AppSettings`.
